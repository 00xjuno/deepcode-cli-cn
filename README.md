...
[Deep Code CLI 中文版](https://github.com/00xjuno/deepcode-cli-cn) — 专为 `deepseek-v4` 模型优化的终端 AI 编码助手，全中文汉化，支持深度思考、推理强度控制、Agent Skills 以及 MCP 集成。
...
### **首次运行配置向导**
- 首次启动自动弹出 API Key 配置向导，无需手动编辑 JSON 文件。
- 输入 DeepSeek API Key 后自动保存，即刻使用。

### **DeepSeek 余额显示**
- 状态栏实时显示 DeepSeek 账户余额，每 1 分钟自动刷新。
- 余额数据调用官方 `/user/balance` API，准确可靠。
...
- 在 GitHub Issues 上报告错误或请求功能 (https://github.com/00xjuno/deepcode-cli-cn/issues)
...
- 在 GitHub 上给我们一个 Star (https://github.com/00xjuno/deepcode-cli-cn)
- 向我们提交反馈和建议
- 分享给你的朋友和同事
